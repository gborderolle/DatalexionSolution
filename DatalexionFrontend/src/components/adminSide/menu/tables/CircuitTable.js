...
import { urlCircuitUpdate } from "../../../../endpoints";
import { CircuitGetMunicipality } from "../../../../utils/auxiliarFunctions";
...
const CircuitTable = (props) => {
  ...
  // Redux
  const circuitList = useSelector((state) => state.generalData.circuitList);
  ...
  useEffect(() => {
    setPageCount(Math.ceil(circuitList.length / itemsPerPage));
  }, [circuitList, itemsPerPage]);
  ...
  const openModal = (circuit = null) => {
    ...
    if (circuit) {
      ...
      inputReset1(circuit.name);
      inputReset2(circuit.number);
      inputReset3(circuit.address);
    } else {
  ...
  const confirmDelete = async () => {
    if (idToDelete) {
      await removeData(urlCircuit, idToDelete);
      dispatch(fetchCircuitList());
      closeDeleteModal();
    }
  };
  ...
  const formSubmitHandler = async (event) => {
    ...
    try {
      ...
      if (currentEntity) {
        ...
        response = await uploadData(dataToUpload, urlCircuit);
      }
      if (response) {
        dispatch(fetchCircuitList());
        inputReset1();
  ...
  return (
    <div>
      ...
      <CTable striped>
        <CTableHead>
          <CTableRow>
            ...
            <CTableHeaderCell onClick={() => requestSort("number")}>
              Número
            </CTableHeaderCell>
            <CTableHeaderCell onClick={() => requestSort("address")}>
              Dirección
            </CTableHeaderCell>
            <CTableHeaderCell onClick={() => requestSort("address")}>
              Municipio
        ...
        <CTableBody>
          {currentList.map((circuit, index) => {
            return (
              <CTableRow key={circuit.id}>
                <CTableDataCell>{index + 1}</CTableDataCell>
                <CTableDataCell>{circuit.name}</CTableDataCell>
                <CTableDataCell>{circuit.number}</CTableDataCell>
                <CTableDataCell>{circuit.address}</CTableDataCell>
                <CTableDataCell>
                  {CircuitGetMunicipality(circuit, municipalityList)?.name}
                </CTableDataCell>
                <CTableDataCell>
                  <CButton
                    color="dark"
                  ...
                  <CButton
                    color="danger"
                    size="sm"
                    onClick={() => openDeleteModal(circuit.id)}
                    style={{ marginLeft: 10 }}
                  >
                    Eliminar
                  </CButton>
      ...
      <CModal visible={isModalVisible} onClose={closeModal}>
        <CModalHeader>
          <CModalTitle>
            {currentEntity ? "Editar entidad" : "Agregar entidad"}
          </CModalTitle>
        </CModalHeader>
